...
device = torch.device("cuda" if is_cuda else "cpu") # zw
# Set t0
t0 = time.time()
...
scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=1500, eta_min=learning_rate * 0.01)
...
while refine_epoch <= stage_2_epoch:
    refine_epoch += 1
    epoch_it += 1

    for batch in train_boundary_loader:
        it += 1
        loss = trainer.train_step(batch, m=2.0)
