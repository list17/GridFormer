...
class BoundaryPointsField(Field):
    ''' Point Field.
    ...
    '''
    def __init__(self, file_name, boundary, transform=None, unpackbits=False, multi_files=None):
        self.file_name = file_name
    ...
    def load(self, model_path, idx, category):
        ...
        '''
        if self.multi_files is None:
            file_path = os.path.join(model_path, self.boundary, self.file_name)
        else:
            num = np.random.randint(self.multi_files)
            file_path = os.path.join(model_path, self.boundary, '%s_%02d.npz' % (self.file_name, num))

        points_dict = np.load(file_path)
        points = points_dict['points'][points_dict["contrastive"] == 1]
        # Break symmetry if given in float16:
        if points.dtype == np.float16:
            points = points.astype(np.float32)
            points += 1e-4 * np.random.randn(*points.shape)
        ...
        occupancies = points_dict['occupancies']
        if self.unpackbits:
            occupancies = np.unpackbits(occupancies)[points_dict["contrastive"] == 1]
        occupancies = occupancies.astype(np.float32)
