...
def get_trainer(model, optimizer, scheduler, cfg, device, **kwargs):
    ...
    if cfg['data']['points_file'] is not None:
        if input_type != 'pointcloud_crop':
            if mode != 'train_boundary':
                ...
                fields['points'] = data.BoundaryPointsField(
                    cfg['data']['boundary_file'], cfg['data']['boundary'], points_transform,
                    unpackbits=cfg['data']['points_unpackbits'],
                    multi_files=cfg['data']['multi_files']
                )
        else:
