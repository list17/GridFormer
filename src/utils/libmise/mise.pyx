...
import numpy as np
#from libc.stdint cimport int32_t, int64_t ### linux-windows issue
...
cdef class MISE:
    ...
    def query(self):
        ...
        # Convert to numpy
        points_np = np.zeros((points.size(), 3), dtype=np.int64) ### linux-windows issue
        #points_np = np.zeros((points.size(), 3), dtype=np.int64_t)
    ...
    def get_points(self):
        ...
        # points_np = np.zeros((self.grid_points.size(), 3), dtype=np.int64_t)
